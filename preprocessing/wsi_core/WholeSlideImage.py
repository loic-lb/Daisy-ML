...
from .wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, \
    screen_coords, isBlackPatch, isWhitePatch, to_percentiles, local_average, compute_law_feats, filter_ROI, thresh_ROI, \
...
class WholeSlideImage(object):
    def __init__(self, path, ROIsdf, tumorareasfolder, contoursfile=None):
        ...
        self.contoursfile = contoursfile
    ...
    def process_contour(self, cont, patch_level, save_path, patch_size=256, step_size=256,
        ...
        iterable = [(coord, ref_patch_size[0], cont_check_fn, self.path, patch_level, patch_size) for coord in
                    coord_candidates]
    ...
    @staticmethod
    def process_coord_candidate(coord, ref_patch_size, cont_check_fn, path, patch_level, patch_size):
        if WholeSlideImage.isInContours(cont_check_fn, coord, patch_size=ref_patch_size):
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0, 0)):
        ...
        contours_tissue = sorted(contours_tissue, key=lambda x: cv2.contourArea(x[0]), reverse=True)
