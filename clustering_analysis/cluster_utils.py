...
from preprocessing import to_percentiles
from .imports import *
...
def get_clustering_coords(slide_name):
    ...
    """
    hdf5_file_path = os.path.join(PATCHES_PATH, 'patches', slide_name + ".h5")
...
def adapt_attention_to_cluster_coords(attention_coords, cluster_coords, scores, patch_size_big=256,
    ...
    The patches have been selected if they belong to a patch from the attention map and attributed
...
def get_attention_filtered_resnet_feats(dataset, slide_idx, indices):
    ...
    :param slide_idx: slide index of the selected slide
...
def get_resnet_features(df, cohort_names, dataset, ponderate=False, attention_thresh=None):
    """

    :param df: dataframe with slide indices and IHC values
    :param cohort_names: List of cohort names to select
    :param dataset: dataset containing features matrix of each slide
    :param ponderate: multiply each patch feature vector by attention score
    :param attention_thresh: minimum score for a patch to be kept (if None all patches are selected)
    :return: matrix of features for the selected patches, coordinates of these patches on their respective slide
    """
    slides_list = np.concatenate([df[df.cohort == cohort_name].slide_id for cohort_name in cohort_names])
    all_feats = []
    all_feats_coords = []
    for slide_id in tqdm(slides_list):
        feats, feats_coords = construct_feats(df, slide_id, dataset, ponderate, attention_thresh)
        all_feats_coords.append(feats_coords)
        all_feats.append(feats)
    return all_feats, all_feats_coords
...
def visualise_clusters(slide_name, labels, coords_filtered=None, label=None, downscale_lvl=3):
    ...
    WSI_object = WholeSlideImage(os.path.join(SLIDE_PATH, f"{slide_name}.mrxs"), pd.read_csv(ROIS_PATH),
                                 PATHOLOGIST_ANNOTATIONS_PATH)
    wsi = WSI_object.getOpenSlide()
    downscale = wsi.level_downsamples[downscale_lvl]
    vis_level = wsi.get_best_level_for_downsample(downscale)
    file = h5py.File(os.path.join(PATCHES_PATH, 'patches', slide_name + ".h5"), 'r')
    dset = file['coords']
...
def patch_closest_to_centroid(labels, WSI_object, kmeans, feats, n_patch=5, downscale_level=1,
    ...
    :param coords_filtered: specify specific coords of patches among the clustered patches (after attention threshold for instance)
    :return: Images of the closest n_patch to cluster (or specify_label clusters) centroids, list of the cluster in WSI_object
    """
    patch_imgs = []
    ...
    file = h5py.File(os.path.join(PATCHES_PATH, 'patches', WSI_object.name + ".h5"), 'r')
    dset = file['coords']
    if coords_filtered is None:
        coords = dset[:]
    else:
